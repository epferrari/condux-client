	...
	Object.defineProperty(this, 'Data', {
		get: function get() {
			if (typeOf(Data) === 'object') {
				return (0, _vendorLodash_mergePullMapEachReduceUniqJs.merge)({}, Data);
			}
			if (typeOf(Data) === 'array') {
				return (0, _vendorLodash_mergePullMapEachReduceUniqJs.map)(Data, function (itm) {
					return itm;
				});
			}
			return Data;
 ...
 */
	...
	nexus.didConnect.then(function () {
		nexus.joinAndSend("sub", _this.topic);
	...
	this.request = function (constraints) {
		return new Promise(function (resolve, reject) {
			...
			var token = _this.__addResponseListener(function (token, body) {
			...
			nexus.joinAndSend("req", _this.topic, req);
		});
	};
}
...
Frequency.prototype = {
	...
	onmessage: function onmessage(msg) {
		...
		Promise.all((0, _vendorLodash_mergePullMapEachReduceUniqJs.map)(this._listeners_, function (l) {
			return new Promise(function (resolve, reject) {
				l.onMessage && l.onMessage.apply(l.sub, [msg, _this3.Data]);
				resolve();
			});
		}));
	},
 ...
 */
	addListener: function addListener(listener) {
		...
		this.didConnect.then(function () {
			return l.onConnection && l.onConnection.call(l.sub, _this4.Data, _this4.stream);
		});
		return token;
	},
