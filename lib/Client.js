...
ClientNexus.Connect = {
	componentWillMount: function componentWillMount() {
		...
		this.tuneIn = function (topic, onMessage, onClose) {
			_this2._queuedSubscriptions.push(attemptSubscription(_this2, topic, onMessage, onClose));
		};
	},
	...
	componentWillUnmount: function componentWillUnmount() {
		...
		this._queuedSubscriptions.forEach(function (disposer) {
			return disposer();
		});
	}
};
...
function subscribe(frequency, onMessage, onClose) {
	...
	var addSubscriber = frequency.addSubscriber;
	var removeSubscriber = frequency.removeSubscriber;
	...
	if (!this.spectrum[topic]) {
		var queuedSub = function queuedSub(e) {
			...
			if (type === "sub" && _topic === topic) {
				subscribe.call(subscriber, _this3.spectrum[topic], onMessage, onClose);
				_this3.sock.removeEventListener("message", queuedSub);
			}
		};

		this.sock.addEventListener("message", queuedSub);
		return function () {
			return _this3.sock.removeEventListener("message", queuedSub);
		};
	} else {
	...
	createAction: function createAction(actionName) {
		...
		return function (payload) {
			return new Promise(function (resolve, reject) {
				_this4.sock.send(["msg", "CLIENT_NEXUS_ACTIONS", JSON.stringify({
 ...
 */
	registerFrequency: function registerFrequency(topic) {
		...
		var freq = undefined;
		if (!spectrum[topic]) {
			spectrum[topic] = new _FrequencyJs2['default'](topic, this.sock);
			this.connected.then(function () {
				return new Promise(function (resolve, reject) {
					_this6.sock.send(["msg", "CLIENT_NEXUS_ACTIONS", JSON.stringify({
