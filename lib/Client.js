...
ClientNexus.Connect = {
	componentWillMount: function componentWillMount() {
		...
		this.tuneIn = function (topic, onMessage, onClose) {
			_this2._queuedSubscriptions.push(ClientNexus.get().attemptSubscription(_this2, topic, onMessage, onClose));
		};
	},
	...
	createAction: function createAction(actionName) {
		...
		return function (payload) {
			return new Promise(function (resolve, reject) {
				_this3.sock.send(["msg", "CLIENT_NEXUS_ACTIONS", JSON.stringify({
 ...
 */
	registerFrequency: function registerFrequency(topic) {
		...
		if (!spectrum[topic]) {
			...
			this.connected.then(function () {
				return new Promise(function (resolve, reject) {
					_this5.sock.send(["msg", "CLIENT_NEXUS_ACTIONS", JSON.stringify({
	...
	attemptSubscription: function attemptSubscription(subscriber, topic, onMessage, onClose) {
		...
		if (!this.spectrum[topic]) {
			...
			return function () {
				return _this6.sock.removeEventListener("message", queuedSub);
			};
		} else {
