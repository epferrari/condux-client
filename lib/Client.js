...
function _Channel(name, multiplexer) {
	var _this = this;
...
function ClientNexus(prefix) {
	...
	this.multiplexer = new _WebSocketMultiplexJs2['default'](this.sock || new _sockjsClient2['default'](prefix || '/reflux-nexus'));
 ...
 */
	createAction: function createAction(actionName) {
		...
		return function (payload) {
			return _this2.actionChannel.send(JSON.stringify({
 ...
 */
	Channel: function Channel(name) {
		return new _Channel(name, this.multiplexer);
	}
};
