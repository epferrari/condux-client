...
function ClientNexus(url, persistence) {
	...
	this._persistence = (0, _vendorLodash_mergePullMapEachReduceUniqJs.merge)({}, p, persistence);
	this._queue = [];
	this.url = url;
	this.band = {};
	this.connect();
...
ClientNexus.prototype = Object.defineProperties({
 ...
 */
	connect: function connect() {
		...
		this.didConnect.then(function () {
			...
			connecting = false;
			_this2._persistence.onConnection();
			...
			var attemptReconnection = (function () {
				...
				if (attempts) {
					...
					this.didConnect.then(function () {
						...
						_this3._persistence.onReconnect();
					});
				} else {
