 ...
 */
	reconnect: function reconnect(sock) {
		...
		(0, _vendorLodash_mergePullMapEachReduceUniqJs.each)(this.band, function (fq) {
			...
			_this2.didConnect.then(function () {
				_this2.joinAndSend("sub", fq.topic);
				setTimeout(function () {
	...
	createAction: function createAction(actionName) {
		...
		return function (payload) {
			return new Promise(function (resolve, reject) {
				...
				_this3.joinAndSend("msg", CLIENT_ACTIONS, message);
				resolve();
			});
		};
	},
...
ClientNexus.Connect = {
  ...
  */
		this.tuneInto = function (frequency, handlers) {
			...
			listenToFrequency.call(_this5, frequency, handlers);
		};
	},
